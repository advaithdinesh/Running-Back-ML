...
mydata = mydata[["run","attempts"]]
print(mydata)
plt.scatter(mydata["run"], mydata["attempts"])
plt.show()
...
train_x = np.array(train[["run"]])
train_y = np.array(train[["attempts"]])
regr.fit(train_x,train_y)

print("y = ", regr.intercept_, " + ", regr.coef_,"x")

test_x = np.array(test[["run"]])
test_y = np.array(test["attempts"])
...
print("r score: ", (-1*(r2_score(test_y1,test_y)))**(.5))
